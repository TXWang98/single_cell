...
class NN_MF(nn.Module):
    def __init__(self, cell_num, in_feature_num, gene_embedding_dim):
        super(NN_MF, self).__init__()
        self.in_feature_num = in_feature_num
        self.gene_embedding_dim = gene_embedding_dim
        self.activation_func = nn.ReLU()
        self.dropout = nn.Dropout(0.1)
        self.cell_embedding_matrix = nn.Parameter(torch.randn(cell_num, self.gene_embedding_dim))
        self.gene_embedding_matrix = nn.Parameter(torch.randn(self.in_feature_num, self.gene_embedding_dim))
    ...
    def forward(self, x, index):
        ...
        tmp_cell_embedding = self.cell_embedding_matrix[index]
        mat_mul_res = torch.matmul(self.dropout(tmp_cell_embedding), self.dropout(self.gene_embedding_matrix.T))

        return self.cell_embedding_matrix, self.gene_embedding_matrix, mat_mul_res
...
class MultiheadClassifier(nn.Module):
    ...
    def forward(self, x):
        batch_size = x.shape[0]
        multi_head_att = [att(x) for att in self.attention_head_list]
...
class MultilayerClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        return res, cell_embedding_final
